...
`AGENTS.md` is the canonical instruction file for every coding agent used in
this repository. `CLAUDE.md` is retained only as a compatibility entry point
and points here.
...
- Use the debugging, test, review, and commit tools available in the active
  agent environment. Do not make routine investigation or read-only work
  contingent on a tool that is unavailable.
- Treat unavailable optional integrations as non-blocking unless the requested
  work specifically needs that integration.
